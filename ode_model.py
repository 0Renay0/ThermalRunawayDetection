...
def rhs(t, c):
    ...
    dCAdt = -R1 - (cfg.kla2 * (CA_star_g - nCA_gas / cfg.Vheadspace) / 1000.0) * gas2liq_factor - (CA / mr) * dmrdt
    dHPdt = -R1 - 2.0 * R2 - (cfg.kla2 * (HP_star_g - nHP_gas / cfg.Vheadspace) / 1000.0) * gas2liq_factor - (HP / mr) * dmrdt
    dEpdt =  R1 - R3 - (cfg.kla2 * (Ep_star_g - nEp_gas / cfg.Vheadspace) / 1000.0) * gas2liq_factor - (Ep / mr) * dmrdt
    dROdt =  R3 - (RO / mr) * dmrdt
    dWdt  =  R1 + 2.0 * R2 - R3 - (cfg.kla2 * (W_star_g - nW_gas / cfg.Vheadspace) / 1000.0) * gas2liq_factor - (W / mr) * dmrdt
    ...
    return [dCAdt, dHPdt, dEpdt, dROdt, dWdt,
            dO2_liqdt, dn02_gasdt, dnCA_gasdt, dnHP_gasdt, dnEp_gasdt, dnW_gasdt,
            dmrdt, dTrdt, dPdt]